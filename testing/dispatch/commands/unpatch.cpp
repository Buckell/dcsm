...
struct cmd_unpatch_interface final : dcsm::dispatch_interface {
    bool received = false;
    uint16_t universe = 0;
...
TEST(dispatch_commands, unpatch) {
    cmd_unpatch_interface itf;
    dcsm::dispatch dsp(itf);

    for (auto const& test_value : test_values) {
        itf.received = false;

        dsp.process_command("unpatch " + std::to_string(test_value));
