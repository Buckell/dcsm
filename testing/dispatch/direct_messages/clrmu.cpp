...
constexpr uint16_t universe_number = 5;

struct clrmu_interface final : dcsm::dispatch_interface {
    bool received = false;
...
TEST(dispatch_direct_messages, clrmu) {
    clrmu_interface itf;
    ...
    dcsm::message_header const header { 0x000D, static_cast<uint16_t>(data.size() - 5) };
    memcpy(buf + 1, &header, sizeof(header));

    // Copy universe number into buffer.
    memcpy(buf + 5, &universe_number, sizeof(uint16_t));
