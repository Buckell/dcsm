...
struct getmu_interface final : dcsm::dispatch_interface {
    bool received = false;

    void dcsm_getmu(dcsm::command_context &a_ctx, uint16_t const a_universe) override {
        received = true;
...
TEST(dispatch_direct_messages, getmu) {
    getmu_interface itf;
    ...
    dcsm::message_header const header { 0x000C, static_cast<uint16_t>(data.size() - 5) };
