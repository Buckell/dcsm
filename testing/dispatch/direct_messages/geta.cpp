...
struct geta_interface final : dcsm::dispatch_interface {
    std::vector<dcsm::address_pack> addresses;
    bool received = false;

    void dcsm_geta(dcsm::command_context &a_ctx, std::vector<dcsm::address_pack> const &a_addresses) override {
        received = true;
...
TEST(dispatch_direct_messages, geta) {
    geta_interface itf;
    ...
    dcsm::message_header const header { 0x0015, static_cast<uint16_t>(data.size() - 5) };
